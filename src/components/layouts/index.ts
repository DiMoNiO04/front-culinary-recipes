export { default as SecondHeader } from './secondHeader/SecondHeader';
export { default as MainHeader } from './mainHeader/MainHeader';
export { default as Footer } from './footer/Footer';
export { default as Header } from './header/Header';
export { default as SearchPanel } from './searchPanel/SearchPanel';
export { default as AdminModerMain } from './adminModerMain/AdminModerMain';
export { default as CreateUpdatePageLayout } from './createUpdatePageLayout/CreateUpdatePageLayout';
